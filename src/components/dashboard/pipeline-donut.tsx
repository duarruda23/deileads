  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-slate-800 bg-slate-900">
      <header className="border-b border-slate-800 px-5 py-4">
        <h2 className="text-sm font-semibold text-white">{t('pipelineDonut.title')}</h2>
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <>
            <Donut data={data} currency={currency} totalLabel={t('pipelineDonut.total')} />
              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-slate-500 tabular-nums">
                    {s.dealCount} {s.dealCount === 1 ? t('pipelineDonut.deal') : t('pipelineDonut.deals')}
                  </span>
...
// ------------------------------------------------------------
function Donut({ data, currency, totalLabel }: { data: PipelineDonutData; currency: string; totalLabel: string }) {
  const size = 200
  const r = 80
  const ringWidth = 18
  const cx = size / 2
