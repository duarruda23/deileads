  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter((c) => {
        ...
        const instagram = c.contact?.instagram_username?.toLowerCase() ?? '';
      ...
      <div className="space-y-2 border-b border-slate-800 p-3">
        <div className="relative">
          ...
          <Input
            ...
            className="focus:border-primary/50 border-slate-700 bg-slate-800 pl-9 text-sm text-white placeholder-slate-500"
          />
        </div>
          ...
          >
            {FILTER_OPTIONS.map((opt) => (
              <DropdownMenuItem
                ...
                className={cn(
                  'text-sm',
                  filter === opt.value ? 'text-primary' : 'text-slate-300'
                )}
              >
                {opt.label}
              </DropdownMenuItem>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isActive && 'border-primary border-l-2 bg-slate-800/70'
      )}
    >
      {/* Avatar */}
        ...
        <div className="mt-0.5 flex items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-1.5">
            {conversation.channel === 'instagram' ? (
              <Camera className="size-3 shrink-0 text-fuchsia-400" />
            ) : (
